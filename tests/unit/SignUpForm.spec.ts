...
describe("SignUpForm.vue", () => {
  it("renders an error when username contains forbidden characters", async () => {
    ...
    await Vue.nextTick()
    expect(wrapper.find(".error_username").text()).toContain("Please enter a valid username")
  })
...
describe("SignUpForm.vue", () => {
  it("renders an error when full name is not letter only", async () => {
    ...
    wrapper.find({ref: "btn-submit"}).trigger('click')
    await Vue.nextTick()
    expect(wrapper.find(".error_fullname").text()).toContain("Please enter a valid name")
  })
...
describe("SignUpForm.vue", () => {
  it("renders an error when number is not integer only", async () => {
    ...
    wrapper.find({ref: "btn-submit"}).trigger('click')
    await Vue.nextTick()
    expect(wrapper.find(".error_number").text()).toContain("Please enter a valid number")
  })
...
describe("SignUpForm.vue", () => {
  it("renders an error when email format is not correct", async () => {
    ...
    wrapper.find({ref: "btn-submit"}).trigger('click')
    await Vue.nextTick()
    expect(wrapper.find(".error_email").text()).toContain("Please enter a valid email address")
  })
...
describe("SignUpForm.vue", () => {
  it("renders an error when password doesn't contain atleast 8 characters", async () => {
    ...
    wrapper.find({ref: "btn-submit"}).trigger('click')
    await Vue.nextTick()
    expect(wrapper.find(".error_password").text()).toContain("Please enter a valid password")
  })
...
describe("SignUpForm.vue", () => {
  it("renders an error when password validation doesn't match", async () => {
    const wrapper = factory({ password: "AzharDifa99", passwordValidation: "AzharDifa88" })
    wrapper.find({ref: "btn-submit"}).trigger('click')
    await Vue.nextTick()
    expect(wrapper.find(".error_password_validation").text()).toContain("Confirm password must match")
  })
  it("doesn't renders an error when password validation is match", async () => {
    const wrapper = factory({ password: "AzharDifa99", passwordValidation: "AzharDifa99" })
    wrapper.find({ref: "btn-submit"}).trigger('click')
    await Vue.nextTick()
